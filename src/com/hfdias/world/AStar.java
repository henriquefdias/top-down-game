...
public class AStar {
	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end) {
		...
		List<Node> openList = new ArrayList<Node>(); // Os espaços disponiveis para o inimigo procurar o player
		List<Node> closedList = new ArrayList<Node>(); // Espaços indisponiveis

		Node current = new Node(start, null, 0, getDistance(start, end));
		openList.add(current);
		while (openList.size() > 0) {
			// Este método tem como primeiro parâmetro a lista a ser ordenada,
			// e o segundo é qual será a base para essa ordenação
			Collections.sort(openList, nodeSorter);
			current = openList.get(0);
			if (current.tile.equals(end)) {
				// Chegou ao objetivo, basta retornar o valor
